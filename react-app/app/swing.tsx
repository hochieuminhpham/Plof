...
export default function Swing() {
  ...
  const swingDirection = useRef<{ x: number; y: number; z: number } | null>(
    null,
  );
  ...
  function handleSwing() {
    ...
    // 2. Normalize direction (so it's only direction, not strength)
    const length = Math.sqrt(dir.x * dir.x + dir.y * dir.y);
    const normX = length !== 0 ? dir.x / length : 0;
    const normY = length !== 0 ? dir.y / length : 0;

    // 3. Use hitSpeed as distance multiplier
    const distanceFactor = peakSpeed.current ?? 0;

    // Tune this value for gameplay feel
    const SCALE = 50;

    const moveX = normX * distanceFactor * SCALE;
    const moveY = normY * distanceFactor * SCALE;
    // 4. Update ball position
    const newBall = {
      ...course.ball,
      xCoord: course.ball.xCoord + moveX,
      yCoord: course.ball.yCoord + moveY,
    };

    // 5. Recalculate distance to target
    const newDistance = Math.hypot(
      course.target.xCoord - newBall.xCoord,
    ...
    const subscription = DeviceMotion.addListener((motion) => {
      ...
      if (phaseRef.current === "baseline") {
        ...
        const avg = baselineSamples.current.reduce(
          (acc, v) => ({
            alpha: acc.alpha + v.alpha / n,
            beta: acc.beta + v.beta / n,
            gamma: acc.gamma + v.gamma / n,
          }),
      ...
      if (phaseRef.current === "backswing") {
        ...
        const alpha =
          target > smoothedPower.current
            ? BACKSWING_ALPHA_UP
            : BACKSWING_ALPHA_DN;
        smoothedPower.current =
          smoothedPower.current * (1 - alpha) + target * alpha;
      ...
      if (phaseRef.current === "downswing") {
        ...
        if (linMag > peakSpeed.current) {
          peakSpeed.current = linMag;

          // Capture direction at the strongest moment of the swing
          swingDirection.current = {
            x: lin.x ?? 0,
            y: lin.y ?? 0,
            z: lin.z ?? 0,
        ...
        if (
          settleCount.current >= DONE_SETTLE_COUNT &&
          peakSpeed.current >= DONE_PEAK_MIN
        ) {
  ...
  return (
    <View>
      ...
      <View style={styles.container}>
        <Text>Allowed shots: {course.allowedShots}</Text>
        <Text>Used shots: {course.usedShots}</Text>
